...
Run the exporter, then probe a printer:
...
Use `--config.file=config.yml` only when a printer needs different paths, timeouts, or TLS settings. Start from [`config.example.yml`](config.example.yml), and validate it with:

```bash
go run ./cmd/epson_exporter --config.file=config.yml --config.check
```
